    ...
    class YITH_Panel {
        ...
        public function panel_enqueue( $hook ) {
            ...
            if( $pagenow == $this->_submenu[0] && isset( $_GET['page'] ) && $_GET['page'] == $this->_submenu[4] ) {
                ...
                wp_enqueue_style( 'jquery-ui', $plugin_dir_url . 'assets/css/jquery-ui.css' , array(), '1.10.3' );
        ...
        public function panel_field_content( $field ) {
            ...
            switch( $field['type'] ) {
                ...
                case 'datepicker':
                    ...
                    $echo  = "<input type='text' id='{$id}_date' name='{$name}[date]' value='{$value['date']}' class='medium-text code panel-datepicker' colorpicker='" . __( 'Select a date', 'yith-pre-launch' ) . "' /> - ";
                    $echo .= "<input type='text' id='{$id}_hh' name='{$name}[hh]' value='{$value['hh']}' class='small-text code' colorpicker='" . __( 'Hours', 'yith-pre-launch' ) . "' /> : ";
                    $echo .= "<input type='text' id='{$id}_mm' name='{$name}[mm]' value='{$value['mm']}' class='small-text code' colorpicker='" . __( 'Minutes', 'yith-pre-launch' ) . "' /> : ";
                    $echo .= "<input type='text' id='{$id}_ss' name='{$name}[ss]' value='{$value['ss']}' class='small-text code' colorpicker='" . __( 'Minutes', 'yith-pre-launch' ) . "' />";
                    if( isset($field['description']) && $field['description'] != '' ) {
                        $echo .= "<p class='description'>{$field['description']}</p>";
                    }
                    break;
                ...
                case 'upload':
                    ...
                    $echo .= "  <input type='text' id='{$id}' name='{$name}' value='{$value}' class='regular-text code' /> <input type='button' name='' id='{$id}_button' class='button' value='". __('Upload', 'yith-pre-launch') ."'>";
                    $echo .= '</div>';
                    if( isset($field['description']) && $field['description'] != '' ) {
                        $echo .= "<p class='description'>{$field['description']}</p>";
                    }
                ...
                case 'typography': $value = wp_parse_args( $value, $field['std'] ); ?>
                    <div class="typography_container typography">
                        <div class="option">
                            ...
                            <div class="select-wrapper font-unit">
                                <select class="typography_unit" name="<?php echo $name ?>[unit]" id="<?php echo $id ?>-unit">
                                    <option value="px" <?php selected( $value['unit'], 'px' ) ?>><?php _e( 'px', 'yith-pre-launch' ) ?></option>
                                    <option value="em" <?php selected( $value['unit'], 'em' ) ?>><?php _e( 'em', 'yith-pre-launch' ) ?></option>
                                    <option value="pt" <?php selected( $value['unit'], 'pt' ) ?>><?php _e( 'pt', 'yith-pre-launch' ) ?></option>
                                    <option value="rem" <?php selected( $value['unit'], 'rem' ) ?>><?php _e( 'rem', 'yith-pre-launch' ) ?></option>
                                </select>
                            </div>
                            ...
                            <div class="select-wrapper font-family">
                                <select class="typography_family" name="<?php echo $name ?>[family]" id="<?php echo $id ?>-family" data-instance="false">
                                    ...
                                    <?php else: ?>
                                        <option value=""><?php _e('Select a font family', 'yith-pre-launch') ?></option>
                                    <?php endif ?>
                                </select>
                            </div>
                            ...
                            <div class="select-wrapper font-style">
                                <select class="typography_style" name="<?php echo $name ?>[style]" id="<?php echo $id ?>-style">
                                    <option value="regular" <?php selected( $value['style'], 'regular' ) ?>><?php _e( 'Regular', 'yith-pre-launch' ) ?></option>
                                    <option value="bold" <?php selected( $value['style'], 'bold' ) ?>><?php _e( 'Bold', 'yith-pre-launch' ) ?></option>
                                    <option value="extra-bold" <?php selected( $value['style'], 'extra-bold' ) ?>><?php _e( 'Extra bold', 'yith-pre-launch' ) ?></option>
                                    <option value="italic" <?php selected( $value['style'], 'italic' ) ?>><?php _e( 'Italic', 'yith-pre-launch' ) ?></option>
                                    <option value="bold-italic" <?php selected( $value['style'], 'bold-italic' ) ?>><?php _e( 'Italic bold', 'yith-pre-launch' ) ?></option>
                                </select>
                            </div>
                        ...
                        <div class="font-preview">
                            ...
                            <!-- Refresh -->
                            <div class="refresh_container"><button class="refresh"><?php _e( 'Click to preview', 'yith-pre-launch' ) ?></button></div>
                        </div>
                    </div>
                    <?php
                        global $yith_panel_if_typography;
